...
{
    private String prefix = "product/pmsAttrGroup";
    ...
    @PostMapping("/add")
    @ResponseBody
    public AjaxResult addSave(AttrGroup attrGroup)
    {
        return attrGroupService.save(attrGroup)
                ? AjaxResult.success()
                : AjaxResult.error();
    }
     ...
     * 修改【请填写功能名称】
     */
    @GetMapping("/edit/{attrGroupId}")
    public String edit(@PathVariable("attrGroupId") Long attrGroupId, ModelMap mmap)
    {
        AttrGroup attrGroup = attrGroupService.getById(attrGroupId);
        mmap.put("attrGroup", attrGroup);
        return prefix + "/edit";
    }
    ...
    @PostMapping("/edit")
    @ResponseBody
    public AjaxResult editSave(AttrGroup attrGroup)
    {
        return attrGroupService.updateById(attrGroup)
                ? AjaxResult.success()
                : AjaxResult.error();
    }
        ...
        {
            return attrGroupService.removeByIds(ids)
                    ? AjaxResult.success()
                    : AjaxResult.error();
        }
}